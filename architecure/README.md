...
![概念架构](_media/概念架构.png ":size=800")

## 逻辑架构

逻辑架构视图

![逻辑架构](_media/逻辑架构.png ":size=800")

## 物理架构

### 物理架构视图

![物理架构](_media/物理架构.png ":size=800")
...
![部署架构-测开环境](_media/部署架构-测开环境.png ":size=800")
...
| :--------- | :----- | :-- | :-------- | :------ | :------------: | :-----------: | :--- | :------------------ |
| 云服务器   | 云主机 | 2   | 8         | 50      |       2        |       1       | 1    | 服务运行服务器      |
| 云服务器   | 云主机 | 4   | 16        | 100     |       2        |       1       | 1    | k8s All in One      |
| 云服务器   | 云主机 | 4   | 16        | 100     |       2        |       1       | 1    | 数据库&中间件服务器 |

#### 上云环境

![部署架构-上云环境](_media/部署架构-上云环境.png ":size=800")

特点：

尽量购买使用云服务云产品，服务稳定，运维难度低。

**部署服务器清单**

| 服务器名称 | 类型         | CPU | 内存（G） | 系统磁盘(G) | 额外磁盘(G) | 公网带宽(Mbps) | 带宽(Gps) | 数量 | 部署内容           |
| :--------- | :----------- | :-- | :-------- | :---------- | :---------- | :------------: | :-------: | :--- | :----------------- |
| SLB        | 负载均衡器   |     |           |             |             |       10       |           | 1    | 公网 IP            |
| 云服务器   | 云主机       | 2   | 4         | 50          |             |                |     1     | 2    | nginx              |
| k8s-master | k8s 集群     |     |           |             |             |                |           | 3    | 云托管             |
| k8s-worker | 云主机       | 4   | 16        | 50          |             |       2        |     1     | 2    | workspace 服务组件 |
| k8s-worker | 云主机       | 4   | 32        | 50          |             |       2        |     1     | 2..N | code pod           |
| k8s-worker | 云主机       | 2   | 4         | 50          | >= 50       |       2        |     1     | 2..N | longhorn 存储      |
| MySQL      | 云数据库     | 2   | 4         | 50          |             |                |           | 1    | 云服务             |
| Nacos      | 微服务引擎   | 1   | 2         |             |             |                |           | 1    | 云服务             |
| RocketMQ   | 消息队列     |     |           |             |             |                |           | 1    | 云服务             |
| 镜像仓库   | 容器镜像仓库 |     |           |             |             |                |           | 1    | 云服务             |

#### 企业私有云环境

![部署架构-私有化环境](_media/部署架构-私有化环境.png ":size=800")
...
| 服务器 k8s-worker | 虚拟机 | 2   | 8         | 500     |                |     1     | 2..N | longhorn 存储         |
...
| 分类     | 项目          | 技术选型            | 版本          |
| :------- | :------------ | :------------------ | :------------ |
| 操作系统 | 所有服务器    | Ubuntu Server       | 24.04 LTS x64 |
| 数据库   | 数据库服务器  | MySQL               | 8.0           |
| 中间件   | 消息中间件    | RocketMQ            | 4.9.x         |
| 中间件   | 配置中心      | Nacos               | 2.1.2         |
| 开发语言 | 后端组件      | Java                | 21            |
| 开发语言 | 后端技术      | Java Virtual Thread | java-21       |
| 开发组件 | 后端工程      | SpringBoot          | 3.x           |
| 开发组件 | 后端 WEB 框架 | GraphQL netflix-dgs | 9.0.0         |
| 开发组件 | 微服务框架    | SpringCloud         | 2023.0.1      |
| 开发组件 | 微服务框架    | SpringCloud Alibaba | 2023.0.1.0    |
| 开发组件 | 数据库访问    | Mybatis-Plus        | 3.5.6         |
...
| 数据空间存储 | 用户代码、文件存储                         | 文件     | 分布式存储 | longhorn |
...
![用户数据ER](_media/数据架构-用户数据ER.png ":size=800")
